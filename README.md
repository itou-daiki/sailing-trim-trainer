...
後端カメラは現在のブーム方位へ追従し、ブーム口の520 mm後方からメイン全高の中央へ見上げる透視投影です。カメラ位置と視線は形状コントロールを動かしても固定し、横方向を誇張しません。後方パネルからジブと船体を外し、メインだけの面、ラフ、リーチ、3本のドラフトストライプ、各ピークリングを大きく追えるようにしています。上・中・下の正確な深さとピークは、直下の計測断面と数値で比較します。各断面の揚力・抗力・前進力の代理値を高さ方向へ積分して適合度と推定艇速を算出し、操作優先順位は条件開始時の改善量で決めます。調整中は各操作を艇種固有の固定位置に置いたまま、優先番号と完了表示だけを更新します。マスト曲がりは差を読み取れるよう強調した表示で、実艇のプリベンド実測値ではありません。用語は [`CONTEXT.md`](./CONTEXT.md) に定義しています。
