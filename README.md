...
- 「自由練習」と「問題場面」を独立タブに分け、課題一覧を練習中の三面図・操作盤から切り離して表示
...
- 420固有のチョック、ジブ高さを操作
- 470固有のフォア／アフタープラーを排他的に操作（片側を引くと反対側を解除）し、ジブリード前後も操作
...
- 同じ実寸マスト中心軸から、PLANは前後偏位、SIDEは上下ブラックバンド間の直線基準と実マスト、STERNはTOP / MID / LOWの奥行きmmを同時表示
- マストベンド → メイン断面 → 風圧反力 → マストベンドを反復し、形状とリグ荷重が安定する準定常の双方向連成
...
- Vitest: 126テスト（練習モードのタブ分離、420／470別の船体・リグ寸法、マスト後傾・テーパー・滑らかな帆走中ベンド、セール形状とマスト反力の双方向連成、軽風／張力不足／オーバーベンドのシワ判定、15課題の初期診断と完了可能性、三方向の同一3D面、見かけ風、優先順位、共有URL、極端設定を横断する形状／空力不変条件を含む）
