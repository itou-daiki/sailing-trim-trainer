...
現行420クラス規則F.3.5は外側点距離2400 mm、縦断面55〜89 mm、横断面32〜76 mmを定め、後端金具と10 mm以上の外側リミットマークを要求する。現行470クラス規則F.4.3〜F.4.4は外側点距離2650 mm、縦断面54〜72 mm、横断面38 mm以上を定め、後端金具を許可する。共通3Dモデルでは、420を70 × 55 mm、470を63 × 38 mmの代表的な合法断面とし、外側点の後ろに70 mmの代表後端金具を置く。70 mmはクラス規則値ではなく、真後ろから後端面を観察するための表示仮定である。後端は外周と暗い内側面を別ポリゴンにし、真後ろ0°の投影で面積が0にならないことをテストする。実寸では数ピクセルになるため、同じ投影頂点を6倍にした断面詳細を併記し、本体形状自体は誇張しない。
