...
実装では、メインのクロス幅を各リーチ計測点へ置いて区分補間し、ジブは三辺長からヘッド位置を三角測量する。そこへ操作で変わるドラフト、最大位置、ツイスト、マストベンドを載せる。クラスルールは許容上限であり、North Sails M-12やN14-L18など個別製品の型紙ではないため、表示は「クラスルール代表輪郭」とする。

420のメインバテン位置はクラスルールDiagram 17のヘッドから1220 / 2220 / 3220 / 4220 mmの計測位置を高さへ換算した。470メインは3本という構成とクロス幅計測点に合わせた代表位置を使う。ジブは両クラスの規則が最大3本を許し、North Sailsの現行製品図でも3本構成を確認できるため、代表位置として25 / 50 / 75%へ短いリーチバテンを置く。後方カメラは厳密な艇尾中心ではクローズのセールがほぼ線になるため、形を歪めずにコード、ドラフト線、リーチを読める艇尾風下側35°の正投影へ変更し、画面にも角度を明記する。
