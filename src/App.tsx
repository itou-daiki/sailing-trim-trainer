...
function App() {
  ...
  const changeControl = (control: ControlKey, value: number) => {
    const nextControls = applyControlChange(controls, control, value)
    ...
    <div className="app-shell" id="top">
      ...
      <main>
        ...
        <section className="model-note" aria-label="モデルについて">
          ...
          <p>
            ジブは固定ラフを軸に動き、ラフ・リーチ・フット三辺を維持します。上・中・下のピークは表示メッシュとは別に断面式から計算し、同じ3D点を三方向と断面寸法へ使います。420／470のガイド値から求めた後傾線と、帆走中のテーパー付き立体マストを3視点すべてへ表示します。PLANとSIDEでは実寸の基準線・最大たわみ、真後ろでは奥行きへ重なる同じベンドをTOP / MID / LOWのmmで読みます。SIDEは倍率変形や独立した拡大曲線を使わず、実寸マスト本体とメインのラフを同じ3D中心軸から描きます。点線は公式の測り方に合わせ、上下ブラックバンド間の直線基準です。SIDEを拡大するとTOP / MID / LOWの実寸偏位をマスト上で確認できます。マストベンドがメインの深さを変え、その深さとツイストから生じる風圧反力が再びマストを曲げるため、両者が安定するまで準定常の連成計算を反復します。無操作でもリグテンション・スプレッダー・マストステップによるプリベンドを残し、25 / 50 / 75%の局所応答を折れのない曲率へ変換します。470のフォア／アフタープラーは「両方解除・フォアのみ・アフターのみ」の排他状態とし、片側を引くと反対側を0へ戻します。反対方向への切替指導は、現在効いている側の解除を先にします。クロス上では軽風の短い横ジワ、ラフ張力不足の長い横ジワ、オーバーベンドの斜めジワを別判定します。後方の「実艇」は船体中心線の約2.1艇身後方・水面近くから、トランサム、船体、マスト、ジブ、メインの全体を見上げる透視像です。「形を読む」は同じカメラをセール中心へ切り直し、横方向だけを3倍にしてドラフト差を見やすくします。カメラと3D形状は変わりません。メインのクリューは、規則寸法のブラックバンド前縁から表示値と同じ0–25 mmだけ前へ動きます。メイン基本角は艇速と真風から見かけ風を反復計算し、クラス別クローズ基準・迎角15°・シュラウド上限を連続接続します。速度は上・中・下の断面から揚力・抗力・前進力を積分した学習用の推定値で、実測ポーラやCFDではありません。艇バランスとセンターボードは常に最適と仮定します。
          </p>
        </section>
        ...
        <details className="method-note">
          ...
          <div>
            <p>
              一つの3Dセール面に上・中・下の計測断面とマストの曲がりを与え、その間を全高へ連続させ、上・斜め横・実艇の真後ろから観察します。上と斜め横は正投影、後方は船体中心線上の約2.1艇身後方、トランサム基準より260 mm上に仮想カメラを置き、リグ中央へ見上げる透視投影です。カメラ位置と視線は形状コントロールを動かしても固定します。マスト本体は、ガイドのレーキ寸法から求めた後傾、上部テーパー、局所接線に直交する楕円断面を持つ実寸の閉立体です。PLANは前後偏位、SIDEは倍率を掛けない実寸の立体マストとメインのラフを表示します。点線は上下ブラックバンド間の測定基準で、SIDE拡大時にはTOP / MID / LOWの実寸偏位を同じマスト上へ重ねます。真後ろでは前後変位が視線の奥行きへ重なり、左右の輪郭からは量を読めないため、見かけの横曲がりを捏造せず、同じ3D中心軸から求めたTOP / MID / LOWの奥行きmmをマスト上へ対応表示します。「実艇」では水面、船体、マスト、ジブ、メインを全景表示し、「形を読む」では同じ投影をセール中心へ切り直して横座標だけを3倍にし、ドラフトとツイストの差を可視化します。倍率は画面内へ常時表示します。コード方向の面メッシュは等間隔、各断面の最大ドラフト点は断面式から別に生成し、3本のストライプ、ピークリング、寸法値へ同じ点を使います。マストヒール、メイン下部点、ジブ揚程、船首取付点は420 / 470それぞれの公式寸法から同じ座標系へ置き、ジブは固定ラフの局所基底で回転しながらクラス規則の三辺長とトップ幅を維持する面として生成します。無荷重マスト単体の曲率規則40 mmと帆走中のチューニング量を分離し、全操作を出しても420は55 mm、470は40 mmの基準プリベンドを下限に残します。スプレッダー位置の計測値を含む25 / 50 / 75%の局所応答は、突出や折れを作らない一山の形状保持スプラインで接続します。メインシート／バングの上部荷重、420チョックと470プラーの下〜中部荷重、カニンガムの上部寄り二次作用を同じ立体へ反映します。470プラーはフォアとアフターを同時加算せず、両方解除・フォアのみ・アフターのみの三状態から一つを使います。反対方向へ切り替えるときは現在の側を先に解除し、課題の優先順位もその実操作順に固定します。さらに各断面の深さ・ツイストと風速から分布反力を求め、更新したベンドで断面を再計算する負帰還を安定するまで反復します。カニンガムや分布反力のmm係数はマスト型式別の実測同定値ではなく、公開ガイドと流体構造連成研究に基づく方向・相対強度の学習用近似です。クロスのシワは同じ3D面へ沿わせますが、流体・布の有限要素解析ではなく、画像診断を学ぶための種類別表示です。メインのフットボルトロープ長はタック–クリュー間の直線長に使わず、ブーム外側点（420: 2400 mm、470: 2650 mm）と実寸アウトホール量からクリューを置きます。艇速と真風のベクトルから見かけ風を反復計算し、クローズはNorthのクラス別基準、リーチは見かけ風に対する迎角15°、ブロードはシュラウド位置を上限としてメイン角を連続計算します。各断面の深さ・最大深さ位置・ツイストから揚力係数、抗力係数、見かけ風方向の前進力代理値を積分し、推定艇速へ変換します。<strong>真風＋艇速 → 見かけ風 → 基本角度 → マストベンド ↔ セール形状 → 断面性能 → 推定艇速</strong>を反復する準定常の学習モデルで、実艇の実測ポーラやCFDではありません。
            </p>
            <ul>
              ...
              <li><a href="https://colorcode.northsails.com/sailing/wp-content/uploads/2017/05/470_tuning_guide_e01.pdf" target="_blank" rel="noreferrer">North Sails Japan — 470 Tuning Guide / mast puller and Cunningham</a></li>
              <li><a href="https://www.waterwind.it/new/images/pdf/Manuale_470.pdf" target="_blank" rel="noreferrer">International 470 Manual — mainsheet / vang mast-bend load path</a></li>
              <li><a href="https://damiansaponara.blogspot.com/2007/04/470-tuning-guide-boat-set-up-english.html" target="_blank" rel="noreferrer">470 tuning notes — flexible Superspar top-section Cunningham response</a></li>
