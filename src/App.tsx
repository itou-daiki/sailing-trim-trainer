    ...
    <div className="app-shell" id="top">
      ...
      <main>
        ...
        <section className="model-note" aria-label="モデルについて">
          ...
          <p>
            ジブは固定ラフを軸に動き、ラフ・リーチ・フット三辺を維持します。上・中・下のピークは表示メッシュとは別に断面式から計算し、同じ3D点を三方向と断面寸法へ使います。後方はブーム後端の後方に置いた透視カメラからメイン中央へ見上げます。標準の「形×3」は、同じカメラの横方向だけを3倍にし、セール中心へ画面を切り直して、メインの面、3本の横線、ラフ、リーチの差を読みやすくした学習表示です。カメラと3D形状は変えず、「実視」で誇張のない透視像へ戻せます。メインのクリューは、規則寸法のブラックバンド前縁から表示値と同じ0–25 mmだけ前へ動きます。メイン基本角は艇速と真風から見かけ風を反復計算し、クラス別クローズ基準・迎角15°・シュラウド上限を連続接続します。速度は上・中・下の断面から揚力・抗力・前進力を積分した学習用の推定値で、実測ポーラやCFDではありません。艇バランスとセンターボードは常に最適と仮定します。
          </p>
        </section>
        ...
        <details className="method-note">
          ...
          <div>
            <p>
              一つの3Dセール面に上・中・下の計測断面とマストベンドを与え、その間を全高へ連続させ、上・斜め横・ブーム後方から観察します。上と斜め横は正投影、後方はブーム口の520 mm後方に仮想カメラを置き、メイン全高の中央へ見上げる透視投影です。カメラ位置と視線は形状コントロールを動かしても固定し、ジブと船体を外してメインの面、ラフ、リーチ、3本のストライプへ集中します。標準の形状レンズは、3D面やカメラを作り直さず投影後の横座標だけを3倍にし、ドラフトとツイストの差を可視化します。倍率は画面内へ常時表示し、実視切替では同じ投影を1倍で描きます。コード方向の面メッシュは等間隔、各断面の最大ドラフト点は断面式から別に生成し、3本のストライプ、ピークリング、寸法値へ同じ点を使います。マストヒール、メイン下部点、ジブ揚程、船首取付点は420 / 470それぞれの公式寸法から同じ座標系へ置き、マストはクラス規則内の実寸楕円断面を持つ閉じた立体スパー、ジブは固定ラフの局所基底で回転しながらクラス規則の三辺長とトップ幅を維持する面として生成します。メインのフットボルトロープ長はタック–クリュー間の直線長に使わず、ブーム外側点（420: 2400 mm、470: 2650 mm）と実寸アウトホール量からクリューを置きます。艇速と真風のベクトルから見かけ風を反復計算し、クローズはNorthのクラス別基準、リーチは見かけ風に対する迎角15°、ブロードはシュラウド位置を上限としてメイン角を連続計算します。各断面の深さ・最大深さ位置・ツイストから揚力係数、抗力係数、見かけ風方向の前進力代理値を積分し、推定艇速へ変換します。<strong>真風＋艇速 → 見かけ風 → 基本角度／形状 → 断面性能 → 推定艇速</strong>を反復する準定常の学習モデルで、実艇の実測ポーラやCFDではありません。マスト曲がりは操作量をクラス規則の最大曲率40 mm以内へ換算して表示します。
            </p>
            <ul>
      ...
      <footer>
        <span>TRIM NOTE / TRAINING BUILD 0.16.0</span>
