    ...
    <div className="app-shell" id="top">
      ...
      <main>
        ...
        <section className="model-note" aria-label="モデルについて">
          ...
          <p>
            ジブは固定ラフを軸に動き、ラフ・リーチ・フット三辺を維持します。上・中・下のピークは表示メッシュとは別に断面式から計算し、同じ3D点を三方向と断面寸法へ使います。SIDEでは420／470のガイド値へ校正した帆走中のマストと直線基準をmm表示し、曲がり方向を読む線だけ横変位を12倍にしています。倍率は図中に明記します。クロス上では軽風の短い横ジワ、ラフ張力不足の長い横ジワ、オーバーベンドの斜めジワを別判定します。後方の「実艇」は船体中心線の約2.1艇身後方・水面近くから、トランサム、船体、マスト、ジブ、メインの全体を見上げる透視像です。「形を読む」は同じカメラをセール中心へ切り直し、横方向だけを3倍にしてドラフト差を見やすくします。カメラと3D形状は変わりません。メインのクリューは、規則寸法のブラックバンド前縁から表示値と同じ0–25 mmだけ前へ動きます。メイン基本角は艇速と真風から見かけ風を反復計算し、クラス別クローズ基準・迎角15°・シュラウド上限を連続接続します。速度は上・中・下の断面から揚力・抗力・前進力を積分した学習用の推定値で、実測ポーラやCFDではありません。艇バランスとセンターボードは常に最適と仮定します。
          </p>
        </section>
        ...
        <details className="method-note">
          ...
          <div>
            <p>
              一つの3Dセール面に上・中・下の計測断面とマストの曲がりを与え、その間を全高へ連続させ、上・斜め横・実艇の真後ろから観察します。上と斜め横は正投影、後方は船体中心線上の約2.1艇身後方、トランサム基準より260 mm上に仮想カメラを置き、リグ中央へ見上げる透視投影です。カメラ位置と視線は形状コントロールを動かしても固定します。SIDEの立体マストは実寸形状のまま、曲がりを読むオレンジ線と比較基準の緑点線だけを直線基準から横方向12倍にして重ねます。「実艇」では水面、船体、マスト、ジブ、メインを全景表示し、「形を読む」では同じ投影をセール中心へ切り直して横座標だけを3倍にし、ドラフトとツイストの差を可視化します。倍率は画面内へ常時表示します。コード方向の面メッシュは等間隔、各断面の最大ドラフト点は断面式から別に生成し、3本のストライプ、ピークリング、寸法値へ同じ点を使います。マストヒール、メイン下部点、ジブ揚程、船首取付点は420 / 470それぞれの公式寸法から同じ座標系へ置き、マストはクラス規則内の実寸楕円断面を持つ閉じた立体スパー、ジブは固定ラフの局所基底で回転しながらクラス規則の三辺長とトップ幅を維持する面として生成します。無荷重マスト単体の曲率規則40 mmと帆走中のチューニングベンドを分離し、420はガイドの55–75 mm、470は40–80 mmを低域基準として、バング／チョックまたはプラーの追加荷重まで滑らかに表示します。クロスのシワは同じ3D面へ沿わせますが、流体・布の有限要素解析ではなく、画像診断を学ぶための種類別表示です。メインのフットボルトロープ長はタック–クリュー間の直線長に使わず、ブーム外側点（420: 2400 mm、470: 2650 mm）と実寸アウトホール量からクリューを置きます。艇速と真風のベクトルから見かけ風を反復計算し、クローズはNorthのクラス別基準、リーチは見かけ風に対する迎角15°、ブロードはシュラウド位置を上限としてメイン角を連続計算します。各断面の深さ・最大深さ位置・ツイストから揚力係数、抗力係数、見かけ風方向の前進力代理値を積分し、推定艇速へ変換します。<strong>真風＋艇速 → 見かけ風 → 基本角度／形状 → 断面性能 → 推定艇速</strong>を反復する準定常の学習モデルで、実艇の実測ポーラやCFDではありません。
            </p>
            <ul>
      ...
      <footer>
        <span>TRIM NOTE / TRAINING BUILD 0.18.0</span>
