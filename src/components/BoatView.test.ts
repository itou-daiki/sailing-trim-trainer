...
describe('teaching-oriented before and after comparison', () => {
  it('puts the fixed pre-trim section beside the live post-trim section', () => {
    ...
    expect(markup).toContain('ラフからリーチ／後傾・ベンド')
    expect(markup).toContain('PLAN / 上からの現在のマスト')
    ...
    expect(markup).toContain('SIDEのマストベンド表示')
    expect(markup).toContain('ベンド強調')
    expect(markup).toContain('MAST + MAIN LUFF · BEND ×4')
    expect(markup).toContain('geometry-mast-model is-actual is-side is-bend-emphasized')
    expect(markup).toContain('曲率を4倍で強調し、数値は実寸')
    expect(markup).not.toContain('geometry-side-bend-lens')
    expect(markup).not.toContain('BEND LENS · ×20')
    ...
    expect(markup).toContain('TOP · 奥')
    expect(markup).toContain('MID · 奥')
    expect(markup).toContain('LOW · 奥')
