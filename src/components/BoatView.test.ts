...
describe('teaching-oriented before and after comparison', () => {
  it('puts the fixed pre-trim section beside the live post-trim section', () => {
    ...
    expect(markup).toContain('実寸マスト · 点線は上下ブラックバンドを結ぶ直線基準')
    expect(markup).toContain('上下ブラックバンドを結ぶ直線基準と、帆走中の立体マストを実寸で比較')
    expect(markup).not.toContain('is-bend-emphasized')
    expect(markup).not.toContain('ベンド強調')
    expect(markup).not.toContain('BEND ×4')
